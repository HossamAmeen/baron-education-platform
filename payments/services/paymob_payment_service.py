...
class PaymobPaymentService:
    ...
    def create_paymob_intention(
        ...
        if response.status_code != 201:
            logger.warning("Paymob intention created failed error: {}". format(response.json()))
            return response.status_code, None

        client_secret = response.json().get("client_secret")
        if not client_secret:
            logger.warning("client_secret is not found: {} with status code: {}". format(response.json(), response.status_code))
            return response.status_code, None
